// This code came from https://github.com/espressif/arduino-esp32/blob/master/libraries/DNSServer/src/DNSServer.cpp
// Unfortunately the latest version of the expressive arduino framework  (> 3.1) crashloops, so this had to be 
// backported.
...
*/
  ...
  if ((dnsHeader.OPCode == DNS_OPCODE_QUERY) && requestIncludesOnlyOneQuestion(dnsHeader)) {
      String domainName = getDomainNameWithoutWwwPrefix(static_cast<const unsigned char *>(dnsQuestion.QName), dnsQuestion.QNameLength);
      if (_domainName.isEmpty() || (_domainName.indexOf(domainName) != -1)) {
